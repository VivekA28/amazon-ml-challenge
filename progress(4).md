...
Very selective; insufficient recall alone

EXP-002

Rare name tokens (2 rarest)
...
High recall but candidate explosion; diagnostic only
...
Do not use geocoding or external address databases.

Do not hard-code only US/India.

Every test S1 must appear in the output.

Final matches must be a subset of candidate pairs.

Optimize for F_0.5, not accuracy alone.

Do not neglect singleton/no-match entities.

Validate locally before using a leaderboard submission.

Keep every serious experiment reproducible.
